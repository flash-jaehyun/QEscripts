...
# adapted to become a structure module, June 2019
...
class Geometry(object):
    ...
    def write_xyz(self):
        """ WW: added for interface with ASE and pymatgen """
        with open("geometry.xyz",mode="wt") as mystruct:
           print("%s" % self.natoms,file=mystruct)
           print("%s" % self.comment,file=mystruct)
           ...
           for i in range(self.natoms):
               ...
               print("%3s   %14.10f   %14.10f   %14.10f   %s" % (self.names[i], self.coordinates[i, 0],
                   self.coordinates[i, 1], self.coordinates[i, 2], extra),file=mystruct)
        mystruct.close()
